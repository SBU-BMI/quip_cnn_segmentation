...
pushd $LOCAL
mpirun -np 4 --map-by node cp /home/lot/scratch/data/original.tgz .
mpirun -np 4 --map-by node tar -xvzf original.tgz
popd
...
~/install/bin/mpirun -np 32 --bind-to none --map-by slot -mca pml ob1 -mca btl ^openib singularity exec -B /local --nv $CONTAINER python main.py
