...
def tile_contain_patch(x0, y0, sx0, sy0, x1, y1, sx1, sy1):
    if x1 >= x0 and x1 < x0 + sx0 and \
       x1 + sx1 >= x0 and x1 + sx1 < x0 + sx0 and \
       y1 >= y0 and y1 < y0 + sy0 and \
       y1 + sy1 >= y0 and y1 + sy1 < y0 + sy0:
        return True
    return False
...
def info2patch_xy(segmentation_polygon_folder, x, y, pw):
    for fn in glob(path.join(segmentation_polygon_folder, '*.csv')):
        ...
        pw_y = int(pw_y)
        mpp = float(mpp)
        if tile_contain_patch(px, py, pw_x, pw_y, x, y, pw, pw):
            return fn, x, y, pw, mpp

    print('Patch is not fully contained by a tile.')
    return None
